...
async function readCache(env: Env): Promise<CachedVocabulary | null> {
  try {
    const raw = await env.MEMORY_CONFIG_KV.get(TAG_VOCABULARY_KEY);
    if (!raw) return null;
...
async function rebuildTagVocabulary(env: Env): Promise<string[]> {
  ...
  try {
    await env.MEMORY_CONFIG_KV.put(TAG_VOCABULARY_KEY, JSON.stringify({ tags, rebuiltAt: Date.now() } satisfies CachedVocabulary));
  } catch (e) {
...
export async function rememberTags(env: Env, tags: string[]): Promise<void> {
  try {
    ...
    await env.MEMORY_CONFIG_KV.put(
      TAG_VOCABULARY_KEY,
      JSON.stringify({
        tags: [...merged].sort(),
