  ...
  ctx.waitUntil(
    classifyEntry(content, env, config)
      .then(async ({ importance, canonical, kind }) => {
        await env.MEMORY_DB.prepare(`UPDATE entries SET importance_score = ? WHERE id = ?`).bind(importance, entryId).run();
        if (!kind && !canonical) return;
        const row = await env.MEMORY_DB.prepare(`SELECT tags FROM entries WHERE id = ?`).bind(entryId).first() as Record<string, any> | null;
        if (!row) return;
        ...
        await env.MEMORY_DB.prepare(`UPDATE entries SET tags = ? WHERE id = ?`).bind(JSON.stringify(tags), entryId).run();
      })
