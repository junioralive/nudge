...
export async function forgetEntry(id: string, env: Env): Promise<ForgetResult> {
  ...
  await env.MEMORY_DB.prepare(`DELETE FROM entries WHERE id = ?`).bind(id).run();

  try {
    await env.MEMORY_DB.prepare(`DELETE FROM edges WHERE source_id = ? OR target_id = ?`).bind(id, id).run();
  } catch (e) {
  ...
  try {
    if (vectorIds.length) {
      await env.MEMORY_VECTORIZE.deleteByIds(vectorIds);
    }
  } catch (e) {
...
export async function deprecateEntry(id: string, env: Env): Promise<boolean> {
  ...
  await env.MEMORY_DB.prepare(`UPDATE entries SET tags = ?, vector_ids = ? WHERE id = ?`)
    .bind(JSON.stringify(withStatus(tags, "deprecated")), "[]", id).run();

  try {
    if (vectorIds.length) await env.MEMORY_VECTORIZE.deleteByIds(vectorIds);
  } catch (e) {
...
export async function applyStatus(id: string, status: MemoryStatus, env: Env): Promise<boolean> {
  ...
  const row = await env.MEMORY_DB.prepare(`SELECT tags FROM entries WHERE id = ?`).bind(id).first() as Record<string, any> | null;
  if (!row) return false;
  const tags: string[] = JSON.parse(row.tags ?? "[]");
  await env.MEMORY_DB.prepare(`UPDATE entries SET tags = ? WHERE id = ?`).bind(JSON.stringify(withStatus(tags, status)), id).run();
  return true;
}
