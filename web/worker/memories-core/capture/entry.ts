...
export async function captureEntry(
  ...
  if (dup.status === "flagged" && mergeAction && mergeAction.action !== "keep_both") {
    ...
    if (targetRow) {
      ...
      if (newVectorIds) {
        ...
        const now = Date.now();
        await env.MEMORY_DB.prepare(`UPDATE entries SET content = ?, tags = ?, updated_at = ? WHERE id = ?`)
  ...
  if (contradiction.detected && contradiction.conflicting_id) {
    ...
    const conflictRow = await env.MEMORY_DB.prepare(
    ...
    if (conflictStatus === "canonical") {
      ...
      await env.MEMORY_DB.prepare(`UPDATE entries SET tags = ? WHERE id = ?`)
        .bind(JSON.stringify(withStatus(draftTags, "draft")), id).run();
      try {
        await env.MEMORY_DB.prepare(`UPDATE entries SET contradiction_wins = contradiction_wins + 1 WHERE id = ?`).bind(conflictId).run();
        await env.MEMORY_DB.prepare(`UPDATE entries SET contradiction_losses = contradiction_losses + 1 WHERE id = ?`).bind(id).run();
      } catch (e) {
    ...
    try {
      await env.MEMORY_DB.prepare(`UPDATE entries SET contradiction_wins = contradiction_wins + 1 WHERE id = ?`).bind(id).run();
      await env.MEMORY_DB.prepare(`UPDATE entries SET contradiction_losses = contradiction_losses + 1 WHERE id = ?`).bind(conflictId).run();
    } catch (e) {
      console.error("Contradiction count update failed (non-fatal):", e);
    }
    try {
