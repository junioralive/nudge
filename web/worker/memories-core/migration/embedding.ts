...
export async function readMigration(env: Env): Promise<MigrationState | null> {
  try {
    const raw = await env.MEMORY_CONFIG_KV.get(MIGRATION_KEY);
    if (!raw) return null;
...
async function writeMigration(env: Env, state: MigrationState): Promise<void> {
  await env.MEMORY_CONFIG_KV.put(MIGRATION_KEY, JSON.stringify(state));
}

export async function clearMigration(env: Env): Promise<void> {
  await env.MEMORY_CONFIG_KV.delete(MIGRATION_KEY);
}
...
async function countRemaining(
  ...
  const stmt =
    cursorCreatedAt === null
      ? env.MEMORY_DB.prepare(sql)
      : env.MEMORY_DB.prepare(sql).bind(cursorCreatedAt, cursorCreatedAt, cursorId);
...
export async function runBatch(
  ...
  const page = state.cursorCreatedAt === null
    ? await env.MEMORY_DB.prepare(pageSql(false)).all()
    : await env.MEMORY_DB.prepare(pageSql(true))
