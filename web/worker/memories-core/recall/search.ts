...
export async function recallEntries(
  ...
  if (tag) {
    ...
    const { results: tagRows } = await env.MEMORY_DB.prepare(
    ...
    try {
      for (let i = 0; i < vectorIds.length; i += VECTORIZE_GET_BY_IDS_BATCH) {
        vectors.push(...await env.MEMORY_VECTORIZE.getByIds(vectorIds.slice(i, i + VECTORIZE_GET_BY_IDS_BATCH)));
      }
    } catch (e) {
    ...
    const denseQuery = async (): Promise<{ matches: VectorizeMatch[] }> => {
      try {
        return await env.MEMORY_VECTORIZE.query(values, { topK: vectorizeTopK, returnMetadata: "all", returnValues: true });
      } catch (e) {
    ...
    if (!semanticUnavailable && results.matches.length && results.matches[0].score < cfg.RECALL_WIDEN_THRESHOLD) {
      try {
        results = await env.MEMORY_VECTORIZE.query(values, { topK: 50, returnMetadata: "all", returnValues: true });
      } catch (e) {
  ...
  for (let i = 0; i < candidateIds.length; i += D1_MAX_BOUND_PARAMS) {
    ...
    const { results: rows } = await env.MEMORY_DB.prepare(
  ...
  ctx.waitUntil(
    Promise.all(
      [...d1Map.keys()].filter(id => seedIdSet.has(id)).map(id =>
        env.MEMORY_DB.prepare(`UPDATE entries SET recall_count = recall_count + 1 WHERE id = ?`).bind(id).run()
      )
